...
import {getSettledTransactions} from '../../../services/api';
...
const SettledTransactions = ({navigation}: Props) => {
      ...
      if (
        ...
        if (merchantsContextState?.merchants.length !== 0) {
          try {
            ...
            const settledTransactions = await getSettledTransactions(
            ...
            setLast30DaysData(
              convertTransactions('last30days', settledTransactions),
            );

            setLast12MonthsData(
              convertTransactions('last12months', settledTransactions),
            );

            setLast30DaysTotals(
              calculateTransactionsTotals('last30days', settledTransactions),
            );

            setLast12MonthsTotals(
              calculateTransactionsTotals('last12months', settledTransactions),
            );
          } catch (error) {
            console.error('[get settled-transactions] error: ', error);
          }
        }
      }
    })();
  ...
  const selectedGraph = () => {
    ...
    const status_code = 134;

    navigate('ReportGatewayTransactions', {created_ts, status_code});
  };
