...
import {getToken, updateCC} from '../../../../services/api';
...
const EditCreditCard = ({navigation, route}: Props) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const flag_ = CardValidator.number(token_.first_six).card?.type;
      if (flag_) {
        setFlag(flag_);
      }

      setRunAVSTransaction(token_.run_avs || false);
    })();
  }, []);
              ...
              <View style={tw`flex-1 mt-4`}>
                ...
                <FadedScrollView style={tw`flex-1 z-0 bg-white m-5`} tw={tw}>
                  ...
                  <View style={tw`flex-row justify-between mt-5`}>
                    ...
                    <Text
                      style={tw`text-gray-900 font-medium text-sm font-inter`}>
                      {originalToken?.exp_date
                        ? `${originalToken.exp_date.substring(
                            0,
                            2,
                          )}/${originalToken.exp_date.substring(2, 4)}`
                        : ''}
                    </Text>
                  </View>
                  <Checkbox
                  ...
                  />
                  <Text style={tw`font-inter text-gray-500 text-sm ml-6 -mt-2`}>
